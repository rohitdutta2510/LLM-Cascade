...
def eval_samsum(datadir, preddir, outdir):
    ...
    rawdata = pd.read_csv(os.path.join(datadir, "samsum_test_1k.csv"))
    rawdata = rawdata.sort_values('prompt_text', key = lambda col: col.apply(len))
    gold = rawdata['summary'].apply(str.lower).values.tolist()

    outdata = []

    for dn in sorted([dn for dn in next(os.walk(preddir))[1] if 'samsum_test_1k' in dn]):       
        modelname = dn[dn.rfind('_') + 1:]
        ...
        try:
            with open(os.path.join(preddir, dn, "output.json")) as fp:
                ...
                if 'flan' not in modelname:
                    preddata = [pr[len(raw):] for pr, raw in zip(preddata, rawdata.prompt_text)]
                    # print(preddata[:5])
                    preddata = [re.sub(r'\n', ' ', pr) for pr in preddata]
                    preddata = [re.sub(r'[^A-Za-z0-9\s]', ' ', pr) for pr in preddata]
                    if 'Phi-3-small-8k-instruct' in modelname:
                        preddata = [' '.join(pr.split()[:10]) for pr in preddata]
                    # print(preddata[:5])
                preddata = map(str.lower, preddata) 
...
def eval_boolq(datadir, preddir, outdir):
    rawdata = pd.read_csv(os.path.join(datadir, "boolq_train_1k.csv"))
    rawdata = rawdata.sort_values('prompt_text', key = lambda col: col.apply(len))
    gold = rawdata['label'].values.tolist()
    ...
    for dn in sorted([dn for dn in next(os.walk(preddir))[1] if 'boolq_train_1k' in dn]):       
        modelname = dn[dn.rfind('_') + 1:]
        ...
        try:
            with open(os.path.join(preddir, dn, "output.json")) as fp:
                ...
                if 'flan' not in modelname:
                    ...
                    preddata = [re.sub(r'\n', ' ', pr) for pr in preddata]
                    preddata = [re.sub(r'[^A-Za-z0-9\s]', ' ', pr) for pr in preddata]
                    preddata = [' '.join(pr.split()[:5]) for pr in preddata]
                    # print(preddata[:20])
