...
from transformers import AutoTokenizer, AutoModelForSeq2SeqLM, AutoModelForCausalLM, Phi3ForCausalLM, BitsAndBytesConfig
...
elif 'Phi-3-medium-4k-instruct' in MODEL_PATH and ('boolq' in DATA_PATH or 'mnli' in DATA_PATH):
...
elif 'Phi-3-medium-4k-instruct' in MODEL_PATH or 'Phi-3-medium-128k-instruct' in MODEL_PATH or 'Phi-3-small-128k-instruct' in MODEL_PATH:
...
with open(os.path.join(OUT_DIR, "output.json"), 'w', encoding ='utf8') as fo:
    # ls = []
    # for row in results:
    #     ls.append(" ".join(map(str, row.cpu().numpy())))
    #     ls.append(tokenizer.decode(row, skip_special_tokens=True))
    # json.dump(ls, fo, indent = 4, ensure_ascii=False)
