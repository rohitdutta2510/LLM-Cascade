...
INDIR = '../llm-sust/datasets/datasets_llm_cascade/'
PREDDIR = '../llm-sust/temp_outs/temp_outs_cascade/'
OUTDIR = '../llm-sust/results/performance_dataset_wise_cascade/'
...
def eval_mnli(datadir, preddir, outdir):
    rawdata = pd.read_csv(os.path.join(datadir, "mnli_train_1k.csv"))
    rawdata = rawdata.sort_values('prompt_text', key = lambda col: col.apply(len))
    gold = rawdata['label'].values.tolist()
    ...
    for dn in sorted([dn for dn in next(os.walk(preddir))[1] if 'mnli_train_1k' in dn]):       
        modelname = dn[dn.rfind('_') + 1:]
